...
CRS- GS 84 / UTM zone 15N (EPSG:32615) 
...
For question 2, make four unique combinations:
  ...
  - Your choice: SWIR1-NIR-B (false color agriculture)
  
What does each image allow you to see?

Answer: Image one uses a combination of natural colors, with much of the image being a faded red color. Image two is infrared with many previously darker areas becoming red, and the original faded red transitioning to green. Image 3 is water focus, with water being a deep blue nd other areas red and green. Finally, image four is an agriculture focus, looksing similar to image two but brighter. Image four was tested with "hist" instead of "lin" which resulted in a brighter, more neon image with black shading.
...
plotRGB(landsat_crop, r = 5, g = 4, b = 3, stretch = "lin",
        main = "Color Infrared: NIR-Red-Green") #bright red = healthy veg

#3 NIR-SWIR1-R (false color water focus)
plotRGB(landsat_crop, r = 5, g = 6, b = 4, stretch = "lin",
        main = "Water Focus: NIR-SWIR1-Red") #dark blue= open water/flooding

#4 WIR1-NIR-B (false color agriculture)
plotRGB(landsat_crop, r = 6, g = 5, b = 2, stretch = "lin", 
        main = "Ag Focus: SWIR1-NIR-B")

#Extra: testing "hist"
plotRGB(landsat_crop, r = 6, g = 5, b = 2, stretch = "hist", 
        main = "Ag Focus: SWIR1-NIR-B")
```
...
indeces_stack <- c(ndvi, ndwi, mndwi, wri, swi)

names(indeces_stack) <- c("NDVI", "NDWI", "MNDWI", "WRI", "SWI")

plot(indeces_stack, col = colorRampPalette(c("blue", "white", "red"))(256))

terra::app(indeces_stack, function(x) sum(!is.finite(x)))
```

> Describe the 5 images. How are they similar and where do they deviate?

Answer: NDVI and SWI both exmpahize the stream/flooding in blue, while others make these areas red. All the images except SWI color the entire area while SWI only returns the water areas. NDWI and MNDWI share a versy similar scale, whcih makes sense, with MNDWI having larger areas with only one color.  
...
ndwi_flood  <- app(ndwi,  function(x) ifelse(x > 0, 1, 0)) #cells > 0 = water

mndwi_flood  <- app(mndwi,  function(x) ifelse(x > 0, 1, 0)) #cells > 0 = water

wri_flood  <- app(wri,  function(x) ifelse(x > 1, 1, 0)) #cells > 1 = water

swi_flood  <- app(swi,  function(x) ifelse(x < 5, 1, 0)) #cells < 5 = water
```
...
```{r}
indeces_flood_stack <- app(
  setNames(c(ndvi_flood, ndwi_flood, mndwi_flood, wri_flood, swi_flood),
    c("NDVI Flood", "NDWI Flood", "MNDWI Flood", "WRI Flood", "SWI Flood")
  ),
  function(x) {
    x[is.na(x)] <- 0
    return(x)
...
plot(indeces_flood_stack, col = c("white", "blue"))
```
...
Answer: All 5 images only show the stream/flood areas, though each has minor differences on what is considered the flood area extent. For example, the top left "chunk" of water is silighty bigger in MNDVI and a slightly different shape in SWI.
...
Answer: there are 6 columns and 12192 rows. The data was extracted as a matrix where each pixel is an observation/row and each band of the landsat is a variable/column.

```{r}
extracted_vals <- values(landsat_crop)
...
```{r}
extracted_vals <- na.omit(extracted_vals)
```
...
```{r}
kmeans_12 <- kmeans(extracted_vals, centers = 12)
clusters <- kmeans_12$cluster
```
...
```{r}
#Copy one band to use as a template
cluster_raster <- landsat_crop[[1]]

#Assign cluster values
values(cluster_raster) <- NA
values(cluster_raster)[!is.na(extracted_vals[,1])] <- clusters

plot(cluster_raster, main = "K-means Clusters (k = 12)")
```
...
#For comparision
# K = 6
kmeans_6 <- kmeans(extracted_vals, centers = 6)
clusters2 <- kmeans_6$cluster

cluster_raster2 <- landsat_crop[[1]]
values(cluster_raster2) <- NA
values(cluster_raster2)[!is.na(extracted_vals[,1])] <- clusters2

# K = 18
kmeans_18 <- kmeans(extracted_vals, centers = 18)
clusters3 <- kmeans_18$cluster

cluster_raster3 <- landsat_crop[[1]]
values(cluster_raster3) <- NA
values(cluster_raster3)[!is.na(extracted_vals[,1])] <- clusters3

plot(cluster_raster2, main = "k = 6")
plot(cluster_raster,  main = "k = 12")
plot(cluster_raster3, main = "k = 18")
```
...
```{r}
#Make table
ndwi_table <- table(values(ndwi_flood), values(cluster_raster))

#Which cluster matches the flood the best
flood_cluster <- which.max(ndwi_table[2, ])

#Mask
kmeans_flood <- app(cluster_raster, function(x) {
  ifelse(x == flood_cluster, 1, 0)
})

#Add to flood stack
indeces_flood_stack <- setNames(
  c(ndvi_flood, ndwi_flood, mndwi_flood, wri_flood, swi_flood, kmeans_flood),
  c("NDVI", "NDWI", "MNDWI", "WRI", "SWI", "KMEANS"))
...
```{r}
#Count flooded pixels per layer
flood_counts <- global(indeces_flood_stack, sum, na.rm = TRUE)

#Define pixel area (30m x 30m)
cell_area <- 30 * 30  # 900 m²

#Convert to area (m²)
flood_area_m2 <- flood_counts * cell_area
...
```{r}
flood_agreement <- app(indeces_flood_stack, sum, na.rm = TRUE)
#0 → no method says flood
#1–5 → partial agreement
#6 → all methods agree

library(RColorBrewer)

plot(flood_agreement, col = brewer.pal(9, "Blues"),
     main = "Flood Classification Agreement (Uncertainty)")
```
...
```{r}
#Copy summed raster
flood_agreement_mask <- flood_agreement

#Set 0 values to NA
values(flood_agreement_mask)[values(flood_agreement_mask) == 0] <- NA

mapview(flood_agreement_mask, layer.name = "Agreement")
```
